...
class CustomUser(AbstractBaseUser):
    ...
    def create(email, password, first_name=None, middle_name=None, last_name=None):
        ...
        if len(first_name) <= 20 and len(middle_name) <= 20 and len(last_name) <= 20 and len(email) <= 100 and len(
            ...
            custom_user = CustomUser.objects.create_user(email=email, password=None, first_name=first_name, middle_name=middle_name,
                                     last_name=last_name)
    ...
    def has_module_perms(self, app_label):
        """
        Return True if the user has module-level permissions for the given app_label.
        """
